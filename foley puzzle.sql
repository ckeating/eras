...
,		CASE WHEN iln.REMOVAL_INSTANT<f.HOSP_ADMSN_TIME THEN '*Before Admission'
			 ...
			 WHEN iln.REMOVAL_INSTANT>f.outofroom AND iln.REMOVAL_INSTANT <f.postopday1_begin   THEN '*After out of room before POD1' 
			 WHEN iln.REMOVAL_INSTANT>=f.postopday1_begin AND iln.REMOVAL_INSTANT <f.postopday1_begin   THEN '*After out of room before POD1' 
			 WHEN iln.REMOVAL_INSTANT>f.outofroom AND iln.REMOVAL_INSTANT <f.postopday2_begin  THEN '*POD1' 
			 WHEN iln.REMOVAL_INSTANT>=f.postopday2_begin  AND iln.REMOVAL_INSTANT <f.postopday3_begin  THEN '*POD2' 
			 WHEN iln.REMOVAL_INSTANT>=f.postopday3_begin  THEN '*POD3 or later' 
			 END AS RemovalWindow	 

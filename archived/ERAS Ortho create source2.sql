...
IF OBJECT_ID('RADB.dbo.CRD_ERASOrtho_GivenMedsallactions') IS NOT NULL
	DROP TABLE RADB.dbo.CRD_ERASOrtho_GivenMedsallactions;
...
INTO  RADB.dbo.CRD_ERASOrtho_GivenMedsallactions
...
JOIN radb.dbo.CRD_ERASOrtho_Med_Dim AS meddim ON meddim.medication_id=cm.MEDICATION_ID
...
SELECT  SUM(mf.Num)
FROM CRD_ERASOrtho_MetricDim metdim
JOIN CRD_ERASOrtho_MetDate metdt ON metdim.ID=metdt.MetID
LEFT JOIN CRD_ERASOrtho_MetricFact mf ON metdt.date_key=mf.DateKey AND metdt.MetID=mf.MetricKey
WHERE metdim.MetricName='Total Encounters'
AND metdt.full_date>'3/1/2016'


sp_helptext CRD_ERASOrtho_MetricFact

SELECT * FROM CRD_ERASOrtho_MetricDim metdim

CRD_ERASOrtho_Cases
sp_helptext CRD_ERASOrtho_EncDim_vw

SELECT * FROM     RADB.dbo.CRD_ERASOrtho_EncDim AS f



SELECT * FROM 
clarity.dbo.zc_mar_rslt
WHERE RESULT_C IN (1,102,113,118,119,134,137,142)



---ortho new tableau source

SELECT COUNT(*) FROM
(SELECT DISTINCT PAT_ENC_CSN_ID 
FROM CRD_ERASOrtho_FlowDetail
) AS f 
JOIN 
 (
SELECT DISTINCT PAT_ENC_CSN_ID
FROM  radb.dbo.CRD_ERASOrtho_Cases
) AS c ON c.PAT_ENC_CSN_ID=f.PAT_ENC_CSN_ID


sp_help CRD_ERASOrtho_FlowDetail



SELECT * FROM CRD_ERASOrtho_Cases
WHERE PAT_ENC_CSN_ID=111272828

SELECT * FROM radb.dbo.vw_PatEnc AS vpe WHERE vpe.PAT_ENC_CSN_ID=111272828

--ortho objects
CRD_ERASOrtho_MetricDim
CRD_ERASOrtho_MetDate
CRD_ERASOrtho_MetricFact
sp_help CRD_ERASOrtho_Cases
sp_helptext CRD_ERASOrtho_EncDim_vw
...
EXEC [dbo].[CRD_ERASOrtho_Create_DateDim]
...
SELECT * FROM radb.dbo.CRD_ERASOrtho_MetricDim AS ceomd
 ...
 SELECT * 
 FROM RADB.dbo.CRD_ERASOrtho_FlowDetail f
 LEFT JOIN radb.dbo.CRD_ERASOrtho_Cases AS c ON f.PAT_ENC_CSN_ID=c.PAT_ENC_CSN_ID

SELECT * FROM RADB.dbo.CRD_ERASOrtho_FlowDetail WHERE PAT_ENC_CSN_ID=103294155
SELECT * FROM radb.dbo.CRD_ERASOrtho_Cases  WHERE PAT_ENC_CSN_ID=103294155

SELECT * FROM clarity.dbo.IP_DATA_STORE AS ids WHERE ids.EPT_CSN=103294155
...
,		b.pat_mrn_id
	...
	,   PhaseofCare=CAST(NULL AS VARCHAR(25))
...
UPDATE RADB.dbo.CRD_ERASOrtho_FlowDetail 
SET PhaseOfCare=CASE WHEN preop=1 THEN 'Preop'

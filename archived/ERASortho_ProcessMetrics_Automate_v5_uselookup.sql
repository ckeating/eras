...
--# Ambulate post op day 0		          OK
--# Pre-op multi modal                   ****** VALIDATE
--# Intra-op spinal anesthesia            OK
--# Intra-op Intra-articular injections  *******VALIDATE - none
--# Intra-op departure from protocol      OK
--# Post-op pain management-parenteral 
--# Anti-emetics post op                  OK
--# Foley catheter utilization



SELECT  COUNT(*) AS totalcases,
		SUM(f.ambulatepod0) AS casesambulated,
		AVG(f.pacudelay) AS avgPacuDelay,
		SUM(f.preopmultimodal) preopmultimodal,
        SUM(intraop_spinalanes) AS spinal_anes ,
		SUM(f.intraop_intraartic) AS intraop_intraarticular,        
        SUM(f.intraop_departure) AS intraop_depfromprotocol ,
		SUM(f.postop_antiemetics) AS postop_antiemetic ,
		SUM(f.postop_painmanage_parent) AS postop_parenteral,
		SUM(f.foleycath) foleycaths,
        COUNT(DISTINCT f.LOG_ID) AS distinctcases
...
SELECT c.PAT_NAME,c.PAT_MRN_ID
,c.HOSP_ADMSN_TIME,c.HOSP_DISCH_TIME
,c.LOG_ID
,meddim.MedBrandName
,meddim.MedType
,med.* 
FROM radb.dbo.CRD_ERASOrtho_Cases AS c
LEFT JOIN  radb.dbo.CRD_ERASOrtho_GivenMeds AS med ON med.pat_enc_csn_id = CASE 
																WHEN med.admissioncsn_flag=1 THEN c.PAT_ENC_CSN_ID
																WHEN med.anescsn_flag=1 THEN c.anescsn
																END
LEFT JOIN radb.dbo.CRD_ERASOrtho_Med_Dim AS meddim ON meddim.medication_id = med.MEDICATION_ID
WHERE med.intraop=1
ORDER BY c.LOG_ID,med.TAKEN_TIME

BEGIN tran
UPDATE radb.dbo.CRD_ERASOrtho_Med_Dim
SET MedType = 'Intra-op Departure from Protocol'
WHERE medication_id IN (77009,166538)

WHERE InProtocol='n'
AND AdminTime = 'Intra-Op'
WHERE medication_id IN (77009,166538)
1.	Morphine (PF) – Duramorph (ERX 77009) OR Bupivacaine, Liposomal (Exparel) (ERX 166538) are administered.
...
SELECT SUM(eoc.ambulatepod0),COUNT(*) 
FROM radb.dbo.ERAS_Ortho_Cases AS eoc

WITH base AS(
SELECT ambulate,preop,RECORDED_TIME,intraop,pacu,postop0,Postop_disch,ambulatepod0=CASE WHEN ambulate=1 AND postop0=1 THEN 1 ELSE 0 end
FROM RADB.dbo.CRD_ERASOrtho_FlowDetail 
)SELECT SUM(base.ambulatepod0),COUNT(*)
FROM base


SELECT * FROM RADB.dbo.CRD_ERASOrtho_GivenMeds;
...
--LEFT JOIN  radb.dbo.CRD_ERASOrtho_Cases AS eo
...
--INNER JOIN radb.dbo.CRD_ERASOrtho_Med_Dim AS meddim ON meddim.medication_id=cm.MEDICATION_ID
...
FROM radb.dbo.CRD_ERASOrtho_GivenMeds ORDER BY pat_enc_csn_id,TAKEN_TIME


123502930
124988587

SELECT * FROM radb.dbo.CRD_ERASOrtho_GivenMeds ORDER BY pat_enc_csn_id,TAKEN_TIME
SELECT * FROM radb.dbo.CRD_ERASOrtho_Cases
SELECT * FROM radb.dbo.CRD_ERASOrtho_Med_Dim WHERE medication_id='27697'
SELECT * FROM radb.dbo.CRD_ERASOrtho_Med_Dim ORDER BY MedType,EpicMedName

UPDATE radb.dbo.CRD_ERASOrtho_Med_Dim
SET MedBrandName=rmt.MED_BRAND_NAME
FROM radb.dbo.CRD_ERASOrtho_Med_Dim d
JOIN clarity.dbo.RX_MED_THREE AS rmt ON d.medication_id=rmt.MEDICATION_ID


ALTER TABLE radb.dbo.CRD_ERASOrtho_Med_Dim
ALTER COLUMN medbrandname VARCHAR(254)
SELECT medication_id ,
       MedType ,
	   null AS MedBrandName,
       MedShortName ,
       EpicMedName ,
       MedOrderSource ,
       AdminTime ,
       InProtocol 
INTO radb.dbo.CRD_ERASOrtho_Med_Dim2
FROM radb.dbo.CRD_ERASOrtho_Med_Dim


sp_rename 'CRD_ERASOrtho_Med_Dim2','CRD_ERASOrtho_Med_Dim'


DROP TABLE radb.dbo.CRD_ERASOrtho_Med_Dim


WHERE medication_id IN (149951,150104)
(ERX-149951) OR Hydromorphone IV Orderable (ERX 150104)

BEGIN tran
UPDATE radb.dbo.CRD_ERASOrtho_Med_Dim
SET MedType='Parental'
WHERE medication_id IN (149951,150104)
ROLLBACK
...
LEFT JOIN clarity.dbo.ZC_ADMIN_ROUTE AS zar ON mai.ROUTE_C=zar.MED_ROUTE_C
WHERE mai.MAR_ACTION_C=1
AND mai.MAR_ENC_CSN IN (123502930
,124988587)
ORDER BY cm.NAME
--AND cm.MEDICATION_ID =27697

SELECT * 
FROM clarity.dbo.RX_MED_ONE AS rmo
LEFT JOIN clarity.dbo.RX_MED_TWO AS rmo2 on rmo.MEDICATION_ID=rmo2.MEDICATION_ID
LEFT JOIN clarity.dbo.RX_MED_THREE AS rmo3 ON rmo.MEDICATION_ID=rmo3.MEDICATION_ID

WHERE rmo.MEDICATION_ID=27697
SELECT *
FROM clarity.dbo.CLARITY_MEDICATION AS cm
WHERE cm.MEDICATION_ID=27697

WITH flow AS (
SELECT ridfsd=row_number () OVER(partition BY fsd_id ORDER BY fsd_id)
,	   ridfsdline =ROW_NUMBER () OVER(partition BY fsd_id,line ORDER BY fsd_id)
,* 
FROM RADB.dbo.CRD_ERASOrtho_FlowDetail
)SELECT * FROM flow WHERE flow.ridfsdline>1
ORDER BY fsd_id
...
,* 
FROM RADB.dbo.CRD_ERASOrtho_FlowDetail
ORDER BY PAT_ENC_CSN_ID,FSD_ID,line
...
SELECT * FROM RADB.dbo.CRD_ERASOrtho_FlowDetail
 ...
 ,fs.RECORDED_TIME
	 ,eoc.HOSP_ADMSN_TIME
      ...
      ,eoc.procedurestart 
      ,eoc.procedurefinish 
      ,eoc.procedurecarecomplete 
      ,eoc.postop0_end 
      ,eoc.timeinpacu 
      ,eoc.pacudelay 
      
FROM RADB.dbo.CRD_ERASOrtho_FlowDetail fs
JOIN radb.dbo.CRED_ERASOrtho_Cases AS eoc ON fs.PAT_ENC_CSN_ID=eoc.PAT_ENC_CSN_ID


WITH base AS (
SELECT rid = ROW_NUMBER() OVER( partition BY pat_enc_csn_id ORDER BY pat_enc_csn_id)
		,* 
FROM radb.dbo.ERAS_Ortho AS eo
)SELECT * 
FROM base b
WHERE pat_enc_csn_id IN (SELECT pat_enc_csn_id FROM base WHERE rid>1)
ORDER BY b.PAT_ENC_CSN_ID
...
pt_x3 =  CASE WHEN RTRIM(LTRIM(value)) = 'x3' THEN 1 ELSE 0 END
	  ...
	  FROM ##erasmob s
		...
		ON v.fsd_id=s.fsd_id AND v.line=s.line
...
SELECT * FROM RADB.dbo.CRD_ERASOrtho_FlowDetail a
...
SELECT * FROM ##erasmob
SELECT s.meas_value ,v.Value,s.fsd_id,s.line,s.*
FROM ##erasmob s
...
SELECT * FROM tempdb.INFORMATION_SCHEMA.COLUMNS AS c
WHERE c.TABLE_NAME='##erasmob'

SELECT * FROM dbo.IP_FLO_CUSTOM_LIST AS ifcl
WHERE id='3047745'
...
DECLARE @mv VARCHAR(100)='5 feet;chair to bed;sidesteps'
PRINT @mv

SELECT *
FROM radb.dbo.YNHH_SplitToTable(@mv,';') AS ystt
...
--LDA urinary catheter volume

--csn-115816074

--summary lda information
SELECT  p.PAT_NAME
,       p.PAT_MRN_ID
,       f.hosp_admsn_time
,       f.surgery_date
...
WHERE   f.PAT_ENC_CSN_ID=115816074
--iln.FLO_MEAS_ID IN ( '3048148000', '8148', '8151' )

 SELECT FLO_MEAS_ID,FLO_MEAS_NAME,DISP_NAME, DUPLICATEABLE_YN
  FROM clarity.dbo.IP_FLO_GP_DATA WHERE DISP_NAME ='urethral catheter'

  --lda assessments -- urine output 
...
--clarity.dbo.IP_LDA_NOADDSINGLE AS iln
LEFT JOIN dbo.IP_FLOWSHEET_ROWS AS ifr ON iln.IP_LDA_ID=ifr.IP_LDA_ID
...
WHERE iln.FLO_MEAS_ID IN (SELECT ifgd.FLO_MEAS_ID FROM dbo.IP_FLO_GP_DATA AS ifgd WHERE ifgd.DISP_NAME ='Urethral Catheter')     
AND ifm.FLO_MEAS_ID='661859'
AND p.pat_mrn_id='MR4644318'
ORDER BY iln.pat_enc_csn_id,ifm.RECORDED_TIME
...
WHERE   ifm.FSD_ID=11203025
AND b.PAT_ENC_CSN_ID=115816074
AND ifm.MEAS_VALUE LIKE '%900%'

SELECT * FROM dbo.IP_FLO_GP_DATA AS ifgd
WHERE ifgd.DISP_NAME ='Urethral Catheter'
        

WHERE   
ifm.FLO_MEAS_ID IN ( '10713938' )
...
CREATE PROCEDURE dbo.usp_ERAS_ProcessMetrics
AS
...
SELECT * 
FROM dbo.IP_FLWSHT_MEAS AS ifm
WHERE ifm.FSD_ID=11962527


SELECT * FROM clarity.dbo.IP_FLO_CUSTOM_LIST AS ifcl
WHERE id='3047745'
...
FROM    clarity.dbo.IP_DATA_STORE AS ids
		JOIN radb.dbo.ERAS_Ortho AS  b ON ids.EPT_CSN = b.PAT_ENC_CSN_ID
        LEFT JOIN clarity.dbo.IP_FLWSHT_REC AS ifr ON ids.INPATIENT_DATA_ID = ifr.INPATIENT_DATA_ID
        LEFT JOIN clarity.dbo.IP_FLWSHT_MEAS AS ifm ON ifr.FSD_ID = ifm.FSD_ID
        LEFT JOIN clarity.dbo.IP_FLO_GP_DATA AS ifgd ON ifm.FLO_MEAS_ID = ifgd.FLO_MEAS_ID
        LEFT JOIN clarity.dbo.ZC_VAL_TYPE AS zvt ON zvt.VAL_TYPE_C = ifgd.VAL_TYPE_C
        ...
        WHERE          ifm.FLO_MEAS_ID IN ( '3047745')		        
...
SET preop=CASE WHEN fs.RECORDED_TIME>=eoc.inpreprocedure AND fs.RECORDED_TIME<=COALESCE(eoc.outofpreprocedure,eoc.anesstart) THEN 1 ELSE 0 END
	...
	, postop0=CASE WHEN fs.RECORDED_TIME>=eoc.procedurestart AND fs.RECORDED_TIME<eoc.postop0_end THEN 1 ELSE 0 END
	 ,postop_disch=CASE WHEN fs.RECORDED_TIME>=eoc.procedurefinish AND fs.RECORDED_TIME<=eoc.HOSP_DISCH_TIME THEN 1 ELSE 0 END
      
FROM RADB.dbo.CRD_ERASOrtho_FlowDetail fs
JOIN radb.dbo.ERAS_Ortho_Cases AS eoc ON fs.PAT_ENC_CSN_ID=eoc.PAT_ENC_CSN_ID;


UPDATE radb.dbo.ERAS_Ortho_Cases
SET ambulatepod0=CASE WHEN fs.ambulate=1 AND fs.postop0=1 THEN 1 ELSE 0 end
FROM radb.dbo.ERAS_Ortho_Cases f
JOIN radb.dbo.CRD_ERASOrtho_FlowDetail AS fs ON f.PAT_ENC_CSN_ID=fs.PAT_ENC_CSN_ID;

SELECT * FROM radb.dbo.ERAS_Ortho_Cases
...
SELECT preop=CASE WHEN fs.RECORDED_TIME>=eoc.inpreprocedure AND fs.RECORDED_TIME<=COALESCE(eoc.outofpreprocedure,eoc.anesstart) THEN 1 ELSE 0 END
	, intraop= CASE WHEN fs.RECORDED_TIME>=eoc.inroom AND fs.RECORDED_TIME<=eoc.outofroom THEN 1 ELSE 0 END
	, pacu = CASE WHEN fs.RECORDED_TIME>=eoc.inpacu AND fs.RECORDED_TIME<=eoc.outofpacu THEN 1 ELSE 0 END
	, postop0=CASE WHEN fs.RECORDED_TIME>=eoc.procedurestart AND fs.RECORDED_TIME<eoc.postop0_end THEN 1 ELSE 0 END
	 ,postop_disch=CASE WHEN fs.RECORDED_TIME>=eoc.procedurefinish AND fs.RECORDED_TIME<=eoc.HOSP_DISCH_TIME THEN 1 ELSE 0 END
...
SELECT * 
	  ,procedurestart
	  ,postop0_end=CONVERT(DATETIME,CONVERT(DATE,DATEADD(dd,1,procedurestart)))
FROM radb.dbo.ERAS_Ortho_Cases
SELECT * from RADB.dbo.CRD_ERASOrtho_FlowDetail
...
CREATE INDEX idx_erasfact ON ##erasfact(pat_enc_csn_id)
...
IF object_id('tempdb..##met12flow') IS NOT NULL
	DROP TABLE ##met12flow;
...
FROM    clarity.dbo.IP_DATA_STORE AS ids
        ...
        AND (ifm.RECORDED_TIME>= DATEADD(hh,14,b.pod1_start) AND ifm.RECORDED_TIME< b.pod2_start)
        AND (ifm.MEAS_VALUE IS NOT NULL )
...
--update for metric 12 flowsheet criteria
UPDATE ##erasfact
SET met12flow=1
FROM ##erasfact f
JOIN ##met12flow AS m
...
--SELECT * FROM radb.dbo.ERAS_CaseFact 

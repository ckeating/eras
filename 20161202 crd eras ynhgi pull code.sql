...
--· Cases 1/1/2015 and later based on surgery date
--· primary OR secondary procedure is performed where CPT code is in CPT Code List
--· Locations performed: 'YNH NORTH PAVILION OR','YNH EAST PAVILION OR','YNH SOUTH PAVILION OR','SRC MAIN OR'


--get eligible surgical logs
...
-- intermediate work table to choose chose eligible cases by the following criteria:
--· If case request time is populated:
--	o time difference in hours between case request time    and in room time > 18 hours
--· If case request time is NOT populated
--	o Time difference in hours between case create date and in room time >= 48 hours
--	o OR 
--	o Time difference in hours between case create date and in room time >= 24 hours and <48 hours AND in room time is equal to or later than 6PM
...
IF object_id('tempdb..#eligiblelogs') is not null
	drop table #eligiblelogs; 
...
SELECT   
  ...
  CONVERT(TIME,In_Room.TRACKING_TIME_IN ) AS inroomTime,
...
FROM    clarity.dbo.OR_LOG 
   JOIN #eligiblelogs AS l ON l.log_id=clarity.dbo.OR_LOG.LOG_ID
...
,		foley_suprapubic=CAST(0 AS INT)
...
FROM clarity.dbo.PAT_ENC_HSP AS peh 
  ...
  FROM RADB.dbo.CRD_ERAS_YNHGI_Case AS e
  LEFT JOIN 
  ( SELECT ap.LOG_ID,COUNT(*) AS ct
    FROM RADB.dbo.CRD_ERAS_YNHGI_Case AS c
	JOIN radb.dbo.CRD_ERAS_YNHGI_AllProc AS ap ON c.LOG_ID=ap.LOG_ID
	GROUP BY ap.LOG_ID
	) AS procct ON procct.LOG_ID=e.LOG_ID;

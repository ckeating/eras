...
--SELECT * 
--FROM tempdb..##allprocs AS a
--WHERE LOG_ID IN (SELECT log_id FROM tempdb..##allprocs GROUP BY LOG_ID HAVING COUNT(*)>1)
--ORDER BY LOG_ID
...
--ORDER BY admissioncsn
	...
	--SELECT *
	--FROM radb.dbo.CRD_ERAS_YNHGI_CptList AS ceycl
	--SELECT * FROM INFORMATION_SCHEMA.COLUMNS AS c
	--WHERE TABLE_NAME='CRD_ERAS_YNHGI_CptList'


--SELECT ProjectID FROM radb.dbo.CRD_ERASProject_Dim WHERE DeliveryNetwork_ShortName=@dn AND ProjectShortName=@erasproject

--1. pull EVERYTHING ins Case table
--1a create temp table of HARS - save distinct HAR with a flag for reprocedure in same visit
--2a create temp table of logs - distinct logs with flags for 
--2. save procedure detail into allproc table
--         fields:
--			 log_id
--			 procline
--			 ProcedureCategory
			...
			--OpenVsLaparoscopic
--3. delete all records in case table except for the first logid
--4. check for duplicate logs by HAR and flag temp har table
--5. case table
